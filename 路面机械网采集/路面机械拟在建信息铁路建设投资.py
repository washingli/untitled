...
for p in range(1,41):
    ...
    p = str(p)
    if(p == '1'):
        url = 'http://news.lmjx.net/yhdt/List_146.shtml'
    else:
        url = 'http://news.lmjx.net/yhdt/List_146_'+p+'.shtml'
...
for i in data_csv.index:
    ...
    try:
        ...
        cursor.execute("insert into 路面机械拟在建信息铁路建设投资(工程项目,项目简介,发布日期) values(%s,%s,%s)", (a, b, c))
        # 更新数据
