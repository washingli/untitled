...
k=0
q=0
data_csv=pd.DataFrame(columns=['product','configuration','productshop','sales','price'])
import requests
import re
...
for p in range(1,20):
    ...
    cs_url = 'https://list.tmall.com/search_product.htm?type=pc&q=%CA%D6%BB%FA&search_condition=4&totalPage=19&cat=50024400&sort=s&style=g&from=sn_1_prop&sarea_code=310100&jumpto='+p+'#J_crumbs'
...
#     url = 'https://list.tmall.com/search_product.htm?type=pc&q=%CA%D6%BB%FA&totalPage=100&sort=s&style=g&sarea_code=310100&jumpto=' + p + '#J_Filter'
    ...
    for m in soupfinal:
        ...
        # print(sale3)
print(data_csv)
data_csv.to_csv('H:\\毕业设计\\tianmaoshangchengphone(1-19)4.13.csv')
